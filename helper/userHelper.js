...
module.exports = {
     addUser : (data) => {
        return new Promise(async (resolve, reject) => {
          data.selectedClass = parseInt(data.selectedClass, 10);
    ...
    getUser:()=>{
        return new Promise(async(resolve,reject)=>{
        try {
          let users = await db.get().collection(collections.USER_COLLECTION).find().sort({selectedClass: 1, rollNumber: 1, division: 1}).toArray();
          if(users){
            resolve(users);
          }
        } catch (error) {
